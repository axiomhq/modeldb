...
import { makeRequest, expectJSON, expectCSV, parseCSV } from './helpers/api';
...
describe('/api/models endpoint', () => {
  ...
  describe('Basic functionality', () => {
    ...
    it('should return correct model structure', async () => {
      ...
      expect(model).toMatchObject({
        ...
        model_type: expect.stringMatching(/^(chat|completion|embedding|image|audio|rerank|moderation)$/),
  ...
  describe('Provider filtering', () => {
    it('should filter by single provider', async () => {
      const response = await makeRequest(app, '/api/models?providers=openai');
      const models = await expectJSON(response);
    ...
    it('should filter by multiple providers', async () => {
      const response = await makeRequest(app, '/api/models?providers=openai,anthropic');
      const models = await expectJSON(response);
    ...
    it('should return empty array for non-existent provider', async () => {
      const response = await makeRequest(app, '/api/models?providers=nonexistent');
      const models = await expectJSON(response);
  ...
  describe('Model type filtering', () => {
    it('should filter by single model type', async () => {
      const response = await makeRequest(app, '/api/models?type=chat');
      const models = await expectJSON(response);
    ...
    it('should filter by multiple model types', async () => {
      const response = await makeRequest(app, '/api/models?type=chat,embedding');
      const models = await expectJSON(response);
  ...
  describe('Capability filtering', () => {
    it('should filter by single capability', async () => {
      const response = await makeRequest(app, '/api/models?capability=vision');
      const models = await expectJSON(response);
    ...
    it('should filter by multiple capabilities (AND logic)', async () => {
      const response = await makeRequest(app, '/api/models?capability=vision,function_calling');
      const models = await expectJSON(response);
    ...
    it('should handle unknown capability gracefully', async () => {
      const response = await makeRequest(app, '/api/models?capability=unknown_capability');
      const models = await expectJSON(response);
  ...
  describe('Deprecation filtering', () => {
    it('should filter non-deprecated models', async () => {
      const response = await makeRequest(app, '/api/models?deprecated=false');
      const models = await expectJSON(response);
    ...
    it('should filter deprecated models', async () => {
      const response = await makeRequest(app, '/api/models?deprecated=true');
      const models = await expectJSON(response);
  ...
  describe('Field projection', () => {
    it('should project single field', async () => {
      const response = await makeRequest(app, '/api/models?project=model_id');
      const models = await expectJSON(response);
    ...
    it('should project multiple fields', async () => {
      const response = await makeRequest(app, '/api/models?project=model_id,provider_id,model_type');
      const models = await expectJSON(response);
      
      expect(models.length).toBeGreaterThan(0);
      models.forEach(model => {
        expect(Object.keys(model).sort()).toEqual(['model_id', 'model_type', 'provider_id']);
      });
    });

    it('should ignore non-existent fields in projection', async () => {
      const response = await makeRequest(app, '/api/models?project=model_id,nonexistent_field');
      const models = await expectJSON(response);
  ...
  describe('Fill with zeros', () => {
    it('should replace null values with zeros', async () => {
      const response = await makeRequest(app, '/api/models?fill-with-zeros=true');
      const models = await expectJSON(response);
    ...
    it('should preserve non-null values', async () => {
      const response = await makeRequest(app, '/api/models?providers=openai&fill-with-zeros=true');
      const models = await expectJSON(response);
  ...
  describe('CSV format', () => {
    it('should return CSV format when requested', async () => {
      const response = await makeRequest(app, '/api/models?format=csv&headers=true');
      expect(response.status).toBe(200);
    ...
    it('should handle CSV with projection', async () => {
      const response = await makeRequest(app, '/api/models?format=csv&project=model_id,provider_id&headers=true');
      const csv = await expectCSV(response);
    ...
    it('should handle CSV without headers', async () => {
      const response = await makeRequest(app, '/api/models?format=csv&headers=false&project=model_id');
      const csv = await expectCSV(response);
  ...
  describe('Combined filters', () => {
    it('should combine provider and type filters', async () => {
      const response = await makeRequest(app, '/api/models?providers=openai&type=embedding');
      const models = await expectJSON(response);
    ...
    it('should combine all filters', async () => {
      const response = await makeRequest(app, '/api/models?providers=openai&type=chat&capability=function_calling&deprecated=false');
      const models = await expectJSON(response);
  ...
  describe('Error handling', () => {
    it('should handle invalid query parameters gracefully', async () => {
      const response = await makeRequest(app, '/api/models?invalid_param=value');
      expect(response.status).toBe(200);
    });

    it('should handle malformed deprecated parameter', async () => {
      const response = await makeRequest(app, '/api/models?deprecated=maybe');
      const models = await expectJSON(response);
      expect(Array.isArray(models)).toBe(true);
    });
  });
