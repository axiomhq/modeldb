...
describe('/api/models/:id endpoint', () => {
  let app: OpenAPIHono;
  ...
  describe('Basic functionality', () => {
    it('should return a specific model by ID', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4');
    ...
    it('should return 404 for non-existent model', async () => {
      const response = await makeRequest(app, '/api/models/non-existent-model');
    ...
    it('should handle special characters in model ID', async () => {
      const response = await makeRequest(app, '/api/models/gpt-3.5-turbo-0301');
      expect(response.status).toBe(200);
  ...
  describe('Field projection', () => {
    it('should project single field', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?project=model_id');
      const model = await expectJSON(response);
    ...
    it('should project multiple fields', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?project=model_id,provider_id,input_cost_per_million');
      const model = await expectJSON(response);
      
      expect(Object.keys(model).sort()).toEqual(['input_cost_per_million', 'model_id', 'provider_id']);
      expect(model.model_id).toBe('gpt-4');
      expect(model.provider_id).toBe('openai');
    ...
    it('should handle non-existent fields in projection', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?project=model_id,non_existent_field');
      const model = await expectJSON(response);
  ...
  describe('Fill with zeros', () => {
    it('should replace null values with zeros', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?fill-with-zeros=true');
      const model = await expectJSON(response);
    ...
    it('should work with projection', async () => {
      const response = await makeRequest(app, '/api/models/claude-3-opus?project=model_id,cache_read_cost_per_million&fill-with-zeros=true');
      const model = await expectJSON(response);
      
      expect(Object.keys(model).sort()).toEqual(['cache_read_cost_per_million', 'model_id']);
  ...
  describe('CSV format', () => {
    it('should return CSV format for single model', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?format=csv&headers=true');
      expect(response.status).toBe(200);
    ...
    it('should handle CSV with projection', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?format=csv&project=model_id,provider_id&headers=true');
      const csv = await expectCSV(response);
    ...
    it('should handle CSV without headers', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?format=csv&headers=false&project=model_id');
      const csv = await expectCSV(response);
    ...
    it('should return CSV error format for 404', async () => {
      const response = await makeRequest(app, '/api/models/non-existent?format=csv');
      expect(response.status).toBe(404);
  ...
  describe('Model with special properties', () => {
    it('should handle embedding model without output costs', async () => {
      const response = await makeRequest(app, '/api/models/text-embedding-ada-002');
      const model = await expectJSON(response);
    ...
    it('should handle model with cache pricing', async () => {
      const response = await makeRequest(app, '/api/models/claude-3-opus');
      const model = await expectJSON(response);
  ...
  describe('Error handling', () => {
    it('should handle URL encoding in model IDs', async () => {
      ...
      const response = await makeRequest(app, `/api/models/${encodedId}`);
      expect(response.status).toBe(200);
    ...
    it('should validate query parameters', async () => {
      const response = await makeRequest(app, '/api/models/gpt-4?fill-with-zeros=invalid');
      expect(response.status).toBe(200);
