...
describe('/api/providers endpoints', () => {
  let app: OpenAPIHono;
  ...
  describe('GET /api/providers', () => {
    describe('Basic functionality', () => {
      it('should list all providers', async () => {
        const response = await makeRequest(app, '/api/providers');
      ...
      it('should return providers in consistent order', async () => {
        const response1 = await makeRequest(app, '/api/providers');
        const providers1 = await expectJSON(response1);
        
        const response2 = await makeRequest(app, '/api/providers');
    ...
    describe('CSV format', () => {
      it('should return CSV format when requested', async () => {
        const response = await makeRequest(app, '/api/providers?format=csv&headers=true');
        expect(response.status).toBe(200);
      ...
      it('should handle CSV without headers', async () => {
        const response = await makeRequest(app, '/api/providers?format=csv&headers=false');
        const csv = await expectCSV(response);
  ...
  describe('GET /api/providers/:id', () => {
    describe('Basic functionality', () => {
      it('should return models for a specific provider', async () => {
        const response = await makeRequest(app, '/api/providers/openai');
        expect(response.status).toBe(200);
      ...
      it('should return 404 for non-existent provider', async () => {
        const response = await makeRequest(app, '/api/providers/non-existent-provider');
      ...
      it('should handle different provider IDs', async () => {
        ...
        for (const providerId of providers) {
          const response = await makeRequest(app, `/api/providers/${providerId}`);
          if (response.status === 200) {
            const models = await expectJSON(response);
            expect(models.every(m => m.provider_id === providerId)).toBe(true);
          }
        }
      });
    });
    ...
    describe('Query parameters', () => {
      it('should support field projection', async () => {
        const response = await makeRequest(app, '/api/providers/openai?project=model_id,model_type');
        const models = await expectJSON(response);
      ...
      it('should support fill-with-zeros', async () => {
        const response = await makeRequest(app, '/api/providers/openai?fill-with-zeros=true');
        const models = await expectJSON(response);
    ...
    describe('CSV format', () => {
      it('should return CSV format for provider models', async () => {
        const response = await makeRequest(app, '/api/providers/openai?format=csv&headers=true');
        expect(response.status).toBe(200);
      ...
      it('should handle CSV with projection', async () => {
        const response = await makeRequest(app, '/api/providers/openai?format=csv&project=model_id,model_type&headers=true');
        const csv = await expectCSV(response);
      ...
      it('should return CSV error format for 404', async () => {
        const response = await makeRequest(app, '/api/providers/non-existent?format=csv');
        expect(response.status).toBe(404);
